...
def main():
    ...
    reqValues = {'used_memory', 'used_memory_human', 'used_memory_rss', 'used_memory_rss_human', 'used_memory_peak',
                 'used_memory_peak_human', 'used_memory_peak_perc', 'used_memory_overhead', 'used_memory_startup',
                 'used_memory_dataset', 'used_memory_dataset_perc', 'allocator_allocated', 'allocator_active',
                 'allocator_resident', 'total_system_memory', 'total_system_memory_human', 'used_memory_lua',
                 'used_memory_lua_human', 'used_memory_scripts', 'used_memory_scripts_human',
                 'number_of_cached_scripts',
                 'maxmemory', 'maxmemory_human', 'maxmemory_policy', 'allocator_frag_ratio', 'allocator_frag_bytes',
                 ...
                 'mem_allocator', 'active_defrag_running', 'lazyfree_pending_objects', 'redis_version',
                 'redis_git_sha1',
                 'redis_git_dirty', 'redis_build_id', 'redis_mode', 'os', 'arch_bits', 'multiplexing_api',
                 'atomicvar_api', 'gcc_version', 'process_id', 'run_id', 'tcp_port', 'uptime_in_seconds',
                 'uptime_in_days', 'hz', 'configured_hz', 'lru_clock', 'executable', 'config_file', 'connected_clients',
                 'client_recent_max_input_buffer', 'client_recent_max_output_buffer', 'blocked_clients', 'loading',
                 'rdb_changes_since_last_save', 'rdb_bgsave_in_progress', 'rdb_last_save_time',
                 'rdb_last_bgsave_status',
                 'rdb_last_bgsave_time_sec', 'rdb_current_bgsave_time_sec', 'rdb_last_cow_size', 'aof_enabled',
                 ...
                 'pubsub_channels',
                 'pubsub_patterns', 'latest_fork_usec', 'migrate_cached_sockets', 'slave_expires_tracked_keys',
                 'active_defrag_hits', 'active_defrag_misses', 'active_defrag_key_hits', 'active_defrag_key_misses',
                 'role', 'connected_slaves', 'slave0', 'master_replid', 'master_replid2', 'master_repl_offset',
                 'second_repl_offset', 'repl_backlog_active', 'repl_backlog_size', 'repl_backlog_first_byte_offset',
                 'repl_backlog_histlen', 'used_cpu_sys', 'used_cpu_user', 'used_cpu_sys_children',
            ...
            for pareNode in pareNodes:
                ...
                if pareNode[4]:
                    memStatus, memResponse = subprocess.getstatusoutput(
                        redisConnectCmd(nodeIP, portNumber, ' info | grep  -e "' + infoParam + ':" '))
                    ...
                    if memStatus == 0:
                        ...
                        print(
                            bcolors.FAIL + '!!! Warning !!!! A problem occurred while memory usage checking !!! '
                                           'nodeID :' + str(nodeNumber) + ' NodeIP:'
                            + nodeIP + ' NodePort:' + portNumber + '' + bcolors.ENDC)

            os.system("clear")
            print(bcolors.HEADER + projectName + ' Redis Cluster  Memory Usage'
